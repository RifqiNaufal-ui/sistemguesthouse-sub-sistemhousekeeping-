...
<div class="row">
    ...
    <form class="form-horizontal" id="yoyo" role="form" method="POST" action="{{ route('order.update', $orderr->id) }}">
    ...
    <tr>
      <td>Customer <input type="text" class="form-control" name="name" value="{{ $orderr->name }}" required=""></td>
      <td>Room</td>
      <select class="form-control select2" name="room_id" value="{{ old('room_id') }}" required="">
        <option value="{{ $orderr->room->id }}">{{ $orderr->room->numberRoom }}</option>
        <option>-- Select Room Number --</option>
        @foreach($rooms as $rs)
        <option value="{{ $rs->id }}">{{ $rs->numberRoom }}</option>
        @endforeach
      </select>
      <td>Department <input type="text" class="form-control" name="department" value="{{ $orderr->department }}" required=""></td>
      <td>Date <input type="date" class="form-control" name="date" value="{{ $orderr->date }}" required=""></td>
      </tr>
      </div>
      </div>
      </div>
    ...
    <table class="table table-striped">
      ...
      <tbody class="neworderbody">
       @foreach($orderr->orderrdetails as $data)
       <input type="hidden" name="idDetailOrder[]" value="{{ $data->id }}">
        <tr>
            <td>
  ...
  <div class="display-4 bg-danger text-white p-2">
    <p>Total : <b class="total">{{ $orderr->total }}</b></p>
